...
**Linux publish job**：`cargo run --bin verify_updater_manifest` 仍会执行 `tauri_build`，因此 `Publish Release` job 也必须先运行 `scripts/ci/install-linux-deps.sh`（含 `libglib2.0-dev`、`pkg-config` 等）。
