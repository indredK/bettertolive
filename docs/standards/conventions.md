...
### F14. 客户端状态与 Controller 模式

> 补充 F4 的「状态归属」约定——服务端数据归 Query，纯客户端状态（弹窗、草稿、选中项、本地偏好）归 zustand / `useState`。本节讲 zustand 的使用模式与 Controller 抽取。

#### F14.1 Zustand 使用规范

- **硬约定**：`store.ts` 只放状态和简单动作（setter），复杂业务编排放 `useXxxController` 或模块内 hook。
- **硬约定**：组件 UI 需要的 store 字段用 **selector 订阅**，禁止 `const store = useXxxStore()` 无 selector 订阅整 store 后再解构——整 store 订阅在每次 state 更新时都会产生新引用，放进 `useCallback` / `useMemo` / `useEffect` 依赖会导致 effect 无限重跑。
- **硬约定**：仅需要 actions（setter）时，用 `useXxxStore((s) => s.setFoo)` 形式按需订阅单个 action，action 引用稳定，不会触发重渲。
- **建议**：多字段订阅（≥5 个）用 `useShallow` 批量订阅，字段少（≤3 个）优先逐个 selector，订阅意图更清晰。
- **建议**：`store.ts` 是否补建按实际共享范围决定：状态被多个页面/组件共享时补 `store.ts`；页面级本地状态留在 controller 或组件 `useState` 即可，不强制补 store 避免过度抽象。

#### F14.2 Controller 模式

当页面组件同时承担状态订阅、派生数据、effect、handler 编排时，抽取 controller hook 让 page.tsx 只保留 JSX 渲染。

- **建议（抽取时机）**：页面组件超过 ~200 行、或含 5 个以上 `useCallback` / `useMemo` / `useEffect` 时，考虑抽取 controller。
- **硬约定（抽取方式）**：把 `page.tsx` 的 store 订阅、本地 UI 状态（`useState`）、派生数据（`useMemo`）、effect、handler（`useCallback`）集中迁入 `hooks/useXxxController.ts`，page.tsx 只保留 JSX 渲染与 `t()` 调用。
- **建议**：子组件可保留原样直接从 `./store` import `useXxxStore` 取 setter——setter 引用稳定，无重渲问题，不必强行迁出。
- **建议**：Controller 返回值只暴露 page.tsx 实际消费的字段；内部使用但页面不需要的字段（如仅用于 effect 同步的 loading 标记）不要返回，避免 page.tsx 解构后未使用触发 lint。

#### F14.3 页面数据层模式（现有约定确认）

当前项目的 `<module>-page-data.ts` 模式已承担部分 controller 职责（数据加载、派生计算），与上述 controller 模式兼容：
- `queries.ts` → Query hooks（服务端数据）
- `<module>-page-data.ts` → 页面级数据组合（Query 结果拼装、派生计算、筛选排序）
- `useXxxController` → 交互编排（handler、本地 UI 状态、effect）——仅在交互复杂时追加
...
> 所有可重复触发的异步操作（提交、加载、删除、刷新等）必须考虑并发安全。

- **硬约定**：可重复点击/提交/启动的异步动作必须做**重入保护**——通过 `isPending`（React Query mutation 自带）或本地 `useState` loading 标记，在进行中禁用按钮/防止重复触发。
- **硬约定**：`useEffect` 中注册的事件监听器、定时器、订阅（`setInterval`、`addEventListener`、WebSocket 等）**必须有清理逻辑**（`return () => cleanup()`），防止内存泄漏和卸载后 setState。
- **硬约定**：异步回调（`setTimeout`、请求回调、事件回调）中操作组件状态时，必须确认组件仍挂载；优先用 React Query 的 `isLoading`/`isError` 驱动 UI，避免手动 isMounted 标记。
- **硬约定**：连续触发的请求（如搜索输入、Tab 切换）必须处理**过期响应**——后发的请求可能先返回，旧响应覆盖新数据。优先用 React Query 的 `queryKey` 自动取消，或用 `AbortController` 手动取消。
- **建议**：复杂异步编排（多步骤、依赖顺序、可取消）优先考虑封装为自定义 hook，不要散落在组件内。
...
- **硬约定**：新增大列表/大表格（预估 > 100 项）优先评估**虚拟化**（虚拟滚动），不默认全量渲染。全量渲染仅在确认数据量小（< 50 项）时接受。
- **建议**：传给重组件的配置对象、回调函数和派生数据使用 `useMemo` / `useCallback` 稳定引用，避免不必要的子组件重渲。
- **建议**：列表项使用稳定的 `key`（业务 ID），不用数组 index；可重排的列表（如拖拽排序）必须用业务 ID 作为 key，否则会出现动画错乱和状态错位。

#### F16.2 代码分包

- **硬约定**：大数据模块（含重型第三方依赖，如 `three`、`cytoscape`、`react-force-graph-3d`）和低频页面**不能默认首屏同步打包**，必须评估按路由/模块动态 `import()` 拆分。
- **建议**：重型依赖（> 50KB gzip）优先评估是否可以懒加载——仅在用户进入对应页面/触发对应功能时加载。
- **建议**：共享 store 被多个子页面复用时，必须明确作用域；不同数据域不共享筛选条件，避免互相干扰。
...
> 当前项目是 Tauri 桌面应用（`live` 模式），同时支持纯浏览器 fallback（`empty` 模式）。平台能力判断必须在边界层统一处理，不散落在业务代码里。

- **硬约定**：平台判断（是否 Tauri 环境、是否可用 IPC、是否有文件系统能力）统一走 `api/config.ts` 的能力检测，不直接在 JSX 和业务代码里写 `typeof window !== "undefined" && window.__TAURI__` 之类的散装判断。
- **硬约定**：浏览器降级、桌面能力缺失、IPC 不可用在**边界层处理**——`api/live/` 是真实实现，`api/fallback/` 是空数据降级，`bettertolive-api.ts` 是统一接口。页面层只消费接口，不感知当前是 live 还是 fallback。
- **硬约定**：fallback 模式必须提供可用的空状态 UI，不能白屏或报错——让用户知道"这是 demo/降级模式"，而不是"应用坏了"。
- **建议**：新增 Tauri 特性时，同时补对应的 fallback 实现（空数组/空对象/默认值），确保纯浏览器环境能正常渲染页面骨架。
...
- **硬约定**：错误码使用 `SCREAMING_SNAKE_CASE`（如 `NOT_FOUND`、`INVALID_INPUT`、`FORBIDDEN_PATH`、`IO_ERROR`、`UNSUPPORTED`），前端可机器判断分类处理。
- **硬约定**：`Mutex` 用 `unwrap_or_else(|e| e.into_inner())` 从 poison 恢复，不直接 `.unwrap()` 打穿。
- **硬约定**：启动期配置读取、迁移、解密失败须显式传播到前端或暴露降级状态，不能仅 `eprintln!` 后继续——用户有权知道应用是否处于受限状态。
- **错误文案**：enum 变体用 `#[error("...")]` 写小写英文短描述带值与原因（如 `invalid channel: {value} (disabled or not found)`），面向开发者排错，**不直接给用户看**（用户文案由前端 i18n 负责）。

#### B2.1 前端错误解析

- **硬约定**：前端统一通过错误解析工具函数处理 IPC 错误，禁止 `typeof error === "string"`、`String(error)`、`error instanceof Error` 之类的散装判断。
- **硬约定**：需本地化的错误在 i18n 的 `errors.<CODE>` 提供文案；解析函数优先取本地化文案，缺失时回退后端 `message`。
...
- **硬约定**：Tauri 命令契约通过 `specta` + `generate:bindings` 自动同步——Rust 端 `#[derive(Type)]` + `#[specta::specta]` 是唯一事实来源，前端 `src/bindings.ts` 自动生成，**不得手写修改**。
- **硬约定**：命令名、参数名、返回 DTO 字段在 TS 与 Rust 两侧通过 specta 保持一致，不允许手动对齐。
- **硬约定**：新增/修改 command 后必须跑 `bun run generate:bindings` 刷新 `src/bindings.ts`；pre-commit 会自动检测 Rust 改动并刷新。
- **硬约定**：改一个 command 至少同步检查：Rust 实现、命令注册（`register_commands`）、specta 派生、bindings 刷新。
...
- **硬约定**：Rust 侧阻塞 I/O（文件读写、数据库查询、计算密集型操作）必须通过 `spawn_blocking` 执行，不得卡在 async 命令主路径——否则会阻塞整个 Tauri 事件循环，导致 UI 冻结。
- **硬约定**：可变后端操作（写文件、删数据、批量修改）必须明确并发策略：是否可重入、是否需要锁、是否支持取消。不确定时默认加锁串行执行。
- **硬约定**：`spawn_blocking` 的 JoinError 转正常错误类型返回，不 panic。

### B8. 批量操作与取消

- **硬约定**：批量任务和取消接口按**幂等设计**——重复调用不能导致状态损坏、数据重复或部分写入。
- **硬约定**：取消后状态必须可恢复至**确定的干净状态**，不能停在中间态（一半删了一半没删）。
- **建议**：长耗时批量操作支持进度反馈（向前端 emit 事件），让用户知道进展而非干等。
...
12. 违反 F14：zustand 无 selector 订阅整 store 后解构放进 hook 依赖（致 effect 无限重跑）、store 里混入复杂业务编排（应抽 controller/use case）
13. 违反 F15：可重复触发的异步动作无重入保护、`useEffect` 注册的订阅/定时器无清理、连续请求无过期响应处理
14. 违反 F16：预估 > 100 项列表不加虚拟化全量渲染、重型依赖（three/cytoscape 等）首屏同步打包不拆分
15. 违反 F17：平台判断散落在业务代码（不统一走 api 边界层）、fallback 模式白屏或报错

**后端**
16. 多步写 / DELETE+INSERT 不包 `write_tx`（或用了 `let conn` 开不了事务）
17. command 路径用 unwrap/expect/可 panic 写法 / Mutex 直接 unwrap 不从 poison 恢复
18. 新代码用 `Result<T, String>` 而非 `thiserror` 类型化错误 / 错误码非 `SCREAMING_SNAKE_CASE`
19. 迁移不包事务、或改已发布的旧迁移版本
20. 改了 command 不跑 `generate:bindings`
21. command 不在 Rust 侧校验前端输入（只靠前端 zod）
22. capability 超出最小必要权限
23. 违反 B7：阻塞 I/O 不走 `spawn_blocking` 卡事件循环、可变后端操作无明确并发策略
24. 违反 B8：批量任务非幂等（重复调用致数据损坏）、取消后状态停在中间态
